---
hide:
  - footer
title: News & Opportunities
---

# News & Opportunities

Funding opportunities, OHDSI network updates, real-world evidence developments, and announcements relevant to Emory's OMOP community.

!!! tip "Pinned — Mar 4, 2026"
    **OMOP Enterprise v1.0.0 Released** [:octicons-arrow-right-24: Read more](posts/2026-03-04-v1-release.md)

## Browse by Category

<div class="grid cards" markdown>

-   :material-cash-multiple:{ .lg .middle } **Funding**

    ---

    NIH RFAs, PCORI, foundation grants, and award announcements.

    *May 6, 2026* — [Broad Pragmatic Studies PCORI Funding Announcement -- Cycle 3 2026](posts/2026-05-06-broad-pragmatic-studies-pcori-funding-announcement----cycle-3-2026.md)

    [:octicons-arrow-right-24: All Funding posts](category/funding/)

-   :material-book-open-variant:{ .lg .middle } **Vocabulary**

    ---

    Vocabulary releases, mapping updates, and concept coverage changes.

    *Feb 27, 2026* — [February 2026 Vocabulary Release Available in Athena](posts/2026-02-27-athena-vocabulary-release-february-2026.md)

    [:octicons-arrow-right-24: All Vocabulary posts](category/vocabulary/)

-   :material-check-decagram:{ .lg .middle } **Data Quality**

    ---

    DQD results, pipeline improvements, and known issue resolutions.

    *Mar 4, 2026* — [OMOP Enterprise v1.0.0 Released](posts/2026-03-04-v1-release.md)

    [:octicons-arrow-right-24: All Data Quality posts](category/data-quality/)

-   :material-server-network:{ .lg .middle } **Infrastructure**

    ---

    Platform changes, new tooling, access updates, and downtime notices.

    *Mar 4, 2026* — [OMOP Enterprise v1.0.0 Released](posts/2026-03-04-v1-release.md)

    [:octicons-arrow-right-24: All Infrastructure posts](category/infrastructure/)

-   :material-human-greeting-proximity:{ .lg .middle } **Community**

    ---

    Training sessions, onboarding, team updates, and research discussions.

    *Mar 2, 2026* — [Best Practices for Coupling OMOP with Machine Learning Metadata](posts/2026-03-02-omop-machine-learning-mlops-best-practices.md)

    [:octicons-arrow-right-24: All Community posts](category/community/)

-   :material-account-group:{ .lg .middle } **OHDSI**

    ---

    Network studies, symposium news, community tools, and workgroup updates.

    *Apr 9, 2026* — [Release of Phenelope - LLM Concept Set Builder](posts/2026-04-09-release-of-phenelope---llm-concept-set-builder.md)

    [:octicons-arrow-right-24: All OHDSI posts](category/ohdsi/)

-   :material-flask:{ .lg .middle } **Real-World Evidence**

    ---

    RWE methods, regulatory developments, and policy changes.

    *Mar 5, 2026* — [AI Triage in Primary Care: Building Safer Real-World Evidence](posts/2026-03-05-ai-triage-primary-care-rwe.md)

    [:octicons-arrow-right-24: All Real-World Evidence posts](category/real-world-evidence/)

</div>

---

## Recent Posts
